...
def _load_config(path: Path, profile: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    try:
        ...
        raise ValueError(
            f"Unknown profile {profile!r}; choose one of: {available}"
        ) from exc
...
def _format_progress(step: int, total_steps: int) -> str:
    """Show actual steps against the requested budget, including batch overshoot."""
    percent = 100.0 * step / total_steps
    return (
        f"steps={step:,}/{total_steps:,} "
        f"({step / 1_000_000:.2f}M/{total_steps / 1_000_000:.2f}M, "
        f"{percent:.1f}%)"
    )
...
def train(args: argparse.Namespace) -> Path:
    ...
    if jax.default_backend() != "gpu" and not args.allow_cpu:
        raise RuntimeError(
            "No JAX GPU detected. On Kaggle use uv sync --extra rl --extra cuda. "
    ...
    for key in ("num_timesteps", "num_envs", "num_evals"):
        ...
        if override is not None:
            ...
            ppo_cfg[key] = override
    num_eval_envs = args.num_eval_envs or int(profile.get("num_eval_envs", 128))
    ppo_cfg.num_eval_envs = num_eval_envs
    if num_eval_envs <= 0:
        raise ValueError("num_eval_envs must be positive")
    if ppo_cfg.batch_size * ppo_cfg.num_minibatches % ppo_cfg.num_envs:
        raise ValueError("num_envs must divide batch_size * num_minibatches (8192).")
    if args.resume is not None and not args.resume.is_dir():
        raise FileNotFoundError(
            "--resume must name an existing numeric checkpoint directory"
        )

    output = args.output.resolve()
    if output.exists() and any(output.iterdir()):
        raise FileExistsError(
            "Use a new output directory for each training invocation."
        )
    ...
    manifest = {
        ...
        "environment": env_cfg,
        "resolved_environment": env._config.to_dict(),
        "resume_from": str(args.resume.resolve()) if args.resume else None,
        "initialization": "checkpoint" if args.resume else "random",
        "resume_semantics": "Restores policy, value and normalizer; optimizer and step count restart",
        "packages": {
    ...
    _write_json(output / "experiment.json", document)
    (output / "model.xml").write_text(Path(env.xml_path).read_text(), encoding="utf-8")
    ...
    def progress(step: int, metrics: dict[str, Any]) -> None:
        record = {
            ...
            "progress_percent": 100.0 * int(step) / total_steps,
        ...
        print(
            f"{_format_progress(int(step), total_steps)} "
            f"eval_reward={reward} wall_time_s={record['wall_time_s']:.1f}",
            flush=True,
        )

    print(
        f"{_format_progress(0, total_steps)} Starting PPO; "
        "compilation may take time. Progress updates arrive at evaluation intervals.",
        flush=True,
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--num-eval-envs", type=int)
